...
def update_scoreboard(csv_path: Path, archive_path: Path, out_csv: Path, out_txt: Path) -> dict:
    ...
    for _, r in arc.iterrows():
        ...
        if d is None or not d.actual:
            continue
        generated = pd.to_datetime(r["generated_at"], errors="coerce")
        if pd.isna(generated) or pd.isna(d.draw_date) or generated.date() >= d.draw_date.date():
            continue
        try:
    ...
    if not scored:
        ...
        out_txt.write_text("NUMBERS3 Live-only leaderboard\nまだ抽せん後に確定したlive予測はありません。\n", encoding="utf-8")
        return {"scored_forecasts": 0, "scored_draws": 0}

    all_scored = pd.DataFrame(scored)
    operational = all_scored.sort_values("generated_at").groupby("target_issue", as_index=False).tail(1)
    ...
    lines = [
        ...
        "開発・バックテストに使った過去回ではなく、予測を保存した後に実際に抽せんされた回だけを採点。",
        "同一回に複数Champion予測がある場合、抽せん日前に保存された最後の予測を採用。", "",
